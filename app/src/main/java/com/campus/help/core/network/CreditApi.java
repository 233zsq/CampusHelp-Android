 ...
 * （后端已与 credit_record 同步并 clamp 0~1000）；本接口只负责变动明细（listByUser）、总变动量（sum）与加减（addRecord）。
...
public interface CreditApi {
    ...
    /** 信用分总变动量。GET /api/credits/sum?userId= ，data = { "sum": N }。 */
    @GET("api/credits/sum")
    Call<ApiResponse<SumResponse>> sum(@Query("userId") long userId);
